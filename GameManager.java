...
public class GameManager {
    ...
    // data for all levels (letters + valid words)
        ...
        public void displayProgress(int levelNum) {
            ...
            for (int i=0;i<targetWords.length;i++) {
                ...
                if (foundStatus[i]) {
                    ...
                    // if word is not found, blanks will be displayed
                    StringBuilder display=new StringBuilder();
                    for (int j=0;j<word.length();j++) {
                        char charToDisplay='_';

                        // Check if a hint applies to this specific letter and is used
                        if (i==0 && j==0 && hintsUsed[0]){ // Hint 1:first letter of first word
                            charToDisplay=hintCharacters[0];
                        } else if(i==2 && j==1 && hintsUsed[1]){ // Hint 2:second letter of third word
                            charToDisplay = hintCharacters[1];
                        } else if (i == targetWords.length - 1 && j == word.length() - 1 && hintsUsed[2]) { // Hint 3:last letter of last word
    ...
    public static boolean playLevel(int levelIndex,Scanner sc) {
        ...
        while (true){
            ...
            switch (result) {
                ...
                case 1 -> System.out.println("Already found that word!");
